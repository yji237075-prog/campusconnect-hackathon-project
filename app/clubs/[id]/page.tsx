...
export default function SubClubsPage() {
  ...
  if (!club) return <div className="min-h-screen flex items-center justify-center text-xl">Club not found</div>;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-blue-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        {club.subClubs.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {club.subClubs.map((sub, index) => (
              <motion.div
                key={sub.id}
              ...
              >
                <Card className="h-full hover:shadow-lg transition-shadow">
                  <CardHeader>
                    ...
                    <CardDescription>
                      President: {sub.president}, Coordinator: {sub.coordinator}
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <Badge>Contact: {sub.contact}</Badge>
                  </CardContent>
                </Card>
              </motion.div>
            ))}
          ...
          <div className="text-center text-muted-foreground text-lg mt-12">
            No sub-clubs available for this club.
          </div>
        )}
      </main>
    </div>
